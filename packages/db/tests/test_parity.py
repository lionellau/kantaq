...
def test_both_dialects_render_every_table() -> None:
    ...
    # 16 collections (8 v0.0.5 + the E13 memory pair + the E06 identity pair +
    # the E12 ticket_relationships + the E17 skill registry pair + the E05-T2
    # conflict_records) + the local infrastructure: schema_version (E02),
    # event_log + sync_cursors (E04), telemetry_events + local_settings (E28).
    assert len(sqlite_ddl) == 21
