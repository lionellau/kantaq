...
| Policy-filtered memory search + the six-state `domain_visibility` (E13-T5, v0.2) | **reuses the in-stack from-scratch ABAC policy engine** (`memory_policy.filter`, MOD-21) behind a new `MemoryService.search` + a pure mapping function | Apache-2.0 (ours) | Golden-rule run 2026-06. The capability is "apply a per-role read policy to a memory search result, and fold `(visibility, review_status, space)` to one of six UX labels." Both are extensions of components already chosen against the bar (E16/MOD-21's ABAC `memory_policy` — OPA/Cerbos/pycasbin/vakt/py-abac all rejected there as wrong-shape or below-bar — and MOD-19's in-stack memory service). No new category, no new dependency: `search` narrows via `list_entries` then calls the existing `filter` so the "never returns an excluded entry" guarantee lives in one enforced path (was re-implemented inline in the `memory_search` MCP tool); `domain_visibility` is a ~10-line pure function (the single source-of-truth mapping, `DOMAIN_VISIBILITIES` the closed vocabulary). No standard claimed beyond the ABAC framing already recorded for MOD-21. |
...
**Confirmed at E05-T2/T3 (Sprint 6–7):** the full conflict engine shipped on the build-from-scratch decision — `detect_merge` + the golden `conflict_vectors.json`, the `conflict_records` collection, mint/resolve, and (E05-T3) the **compare-and-swap (CAS) commit** for resolutions + approved agent proposals (`commit_events(..., cas=True)` → the `p_cas` branch of `events.sql` raises `rebase_required` and commits nothing when a write would contend, so a stale resolution/proposal value never lands). Still no framework adopted; the CAS is ~40 lines of plpgsql + the existing `kantaq.event_conflicts()` scan reused, cross-checked on EphemeralPostgres.

**Decision (E05-T3) — agent-proposal staleness policy (D-17).** When a human approves an agent proposal whose ticket write turns out stale, how aggressively to bounce it is a per-workspace setting `agent_proposal_stale_policy ∈ {auto_rebase (default), strict_rebase}` (runtime config + Settings → Sync, read-only surface). `auto_rebase` bounces only on a genuine field clash (the smooth default — a stale-but-non-conflicting proposal auto-merges, no needless re-approval); `strict_rebase` re-confirms any proposal that raced a change. Either way the agent's stale value never silently wins (§8.5 propose-first). Chosen as head-of-PM call for UX (minimize human re-work) while keeping the safety floor; the unresolved-conflict "ride-flagged" policy (the field keeps its converged LWW value; the `conflict_record` is the audit-and-correct surface) is confirmed and lands its UI lock at E20-T5.
...
### E26 / MOD-27 workspace-metrics-retention (v0.2, recorded at E26-T1)

| Need | Chosen | License | Notes |
|---|---|---|---|
| Rows/bytes estimation (the capacity gauge + replica size) | **built-in database catalog stats — no library** (`kantaq_core.metrics`: Postgres `pg_total_relation_size`/`pg_stat_user_tables`, SQLite `dbstat`/PRAGMA) | Apache-2.0 (ours) | Golden-rule run 2026-06 (E26-T0, confirmed at E26-T1). Category: storage/usage metering. Candidates vs the 5k bar: **prometheus-community/postgres_exporter** 3,545★ (fail — Go daemon, wrong shape, reads the *same* catalog functions), **rapidloop/pgmetrics** 1,097★ (fail — Go CLI), **supabase/supabase-py** 2,520★ (fail — data-plane client, no usage surface), `pg_stat_statements` (built-in but measures query latency, not storage). The catalog functions are the recognized standard (treated like PostgREST/hashlib in the E07/E24 passes): exact, zero-dependency, deterministic — which makes the FR-E26-1 ≤10% gate trivially testable against the seeded profile. **Conclusion: build on the standard catalog stats.** |
| Retention scheduling/execution (audit summarize + `sync_events` compaction) | **build from scratch** on the runtime's sync cycle (`kantaq_core.retention`, throttled via a `local_settings` marker) + backend pg_cron for the `DELETE` | Apache-2.0 (ours) | **APScheduler** 7.5k★/MIT and **Celery** 28.6k★/BSD clear the bar but solve only the trivial 20% (scheduling); Celery needs a broker → breaks local-first + the very cost ceiling this protects. The load-bearing 80% is kantaq-specific (the chain-aware audit summarize must re-anchor the E07 hash chain; the `sync_events` prune must be watermark-safe) — below any library's abstraction. **pg_cron** is the Supabase platform vehicle for the backend `DELETE` (not a vendored library). |

### E23 / MOD-23 Linear importer (v0.2, E23-T3)

| Need | Chosen | License | Notes |
|---|---|---|---|
| Linear export parser (the importer's input) | **stdlib `json`** — no new runtime dependency | Apache-2.0 (ours) | The importer (`kantaq_runtime.linear_import`) consumes a Linear export as JSON and writes through the in-stack event log — same posture as the E23-T2 bundle import (stdlib + the protocol codec). No parsing library is adopted into the stack. |
| XLSX → JSON conversion for the **real** JobWinAI fixture (dev-only smoke) | **openpyxl** (de-facto standard Python xlsx reader, MIT) — a **dev-only** converter, **not** a code-repo dependency | MIT | Golden-rule run 2026-06. The CI gate imports a *synthetic* JobWinAI-shaped export (the real one is private — DEBT-17), so no parser ships. For the local real-data smoke the private XLSX is converted with openpyxl, run ephemerally via `uv run --with openpyxl` (never added to `pyproject`). Candidates: **openpyxl** (the standard, MIT, actively maintained) — chosen; **pandas** 44k★/BSD — far too heavy (pulls numpy) for a one-off dev convert; **xlrd** 2.2k★ — dropped `.xlsx` support, fails the need. No data and no XLSX path enters the public repo. |

### E27 / MOD-15 conformance suite + export round-trip gate (v0.2, E27-T5)
...
| S3-compatible **object storage** for attachments (E25-T3) | **boto3** — an **optional `[s3]` extra**, not a base dependency | Apache-2.0 | Golden-rule run 2026-06 (E25-T3). Object storage *is* a genuinely new capability (vs the filesystem `LocalBlobStore` default, D-32), so ≥3 candidates were weighed against the bar: **boto3** (the de-facto AWS SDK, ~9k★, Apache-2.0, AWS-maintained, talks any S3-compatible endpoint via `endpoint_url` — MinIO / Cloudflare R2) — **chosen**; **minio-py** (MIT, ~800★) — below the star bar and narrower; **raw httpx + SigV4** — build-from-scratch request signing is error-prone, rejected. Kept **optional**: the filesystem store is the zero-dep default and `S3BlobStore` imports boto3 on demand in `from_config`, so the base install gains nothing. The adapter is duck-typed to the S3 client surface (`put`/`get`/`head_object`), so MinIO's own client or any compatible SDK also satisfies it. |

### Sprint 9 (v0.3 depth) — E15 / E20 / E09-T5: golden-rule passes, no new dependencies

| Need | Chosen | Notes |
|---|---|---|
| Dependency graph + blocking-path queries (E15-T2 / MOD-29) | **stdlib** (`collections.deque`, hand-rolled BFS/DFS) | The graph is folded on demand from the v0.1 `ticket_relationships` table — the same reuse precedent as the relation engine's own cycle guard (`_relation_arc` / `_FAMILY_TYPES` reused, so the graph and the guard can never disagree). A graph library (networkx ~14k★, MIT) clears the bar but is disproportionate for a single-family BFS + a DFS cycle-detect over a small in-memory adjacency; build-from-scratch is ~120 lines and avoids the dependency. **No new dep.** |
| Follow-up collection + propose-first tools (E15-T1 / MOD-29) | **reuse** the MOD-04 envelope + the E08/E10 `agent_proposal` seam | No new capability — a follow_up is another syncable collection and its agent writes ride the existing propose-first path (a `kind`-discriminated diff through the one `approve_proposal`). **No new dep.** |
